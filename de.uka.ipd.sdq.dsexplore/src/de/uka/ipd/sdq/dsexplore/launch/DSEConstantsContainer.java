...
public class DSEConstantsContainer {
	...
	/**
	 * whether the rule based search should explore the full search tree up to
	 * the depth given by the number of iterations.
	 */
	...
	/**
	 * This must not be used as a key directly, but the quality attribute has to
	 * be appended.
	 */
	...
	public static final String[] PRIVACY_COUNTRY_LIST_EXTENSION = new String[] { "*.txt" };

	// public static final String[] SECURITY_MODEL_EXTENSION = new String[] {
	// "*.security" };;
	...
	// instead
	// public static final String MEAN_RESPONSE_TIME_QUALITY = "mean response
	// time";
	...
	public enum QualityAttribute {
		...
		PRIVACY_QUALITY {
			public String getName() {
				return "de.uka.ipd.sdq.dsexplore.privacy";
			}
		...
		public static QualityAttribute getQualityAttribute(String name) {
			if (name.equals(PERFORMANCE_QUALITY.getName())) {
				...
				return NQR_QUALITY;
			} else if (name.equals(PRIVACY_QUALITY.getName())) {
				return PRIVACY_QUALITY;
			} else {
				return null;
			}
		}
	...
	// costs";
	// public static final String COST_CONSIDER_OPERATING = "DSE consider
	// opearting costs";
