...
public class BinaryBayesOperator implements BayesianCrossover<DesignDecisionGenotype> {
    ...
    @Inject
    public BinaryBayesOperator(final Rand random) {
        this.random = random;
    }

    @Override
    public Pair<DesignDecisionGenotype> crossover(final DesignDecisionGenotype arg0,
            final DesignDecisionGenotype arg1) {
    ...
    @Override
    public List<DesignDecisionGenotype> crossover(final List<DesignDecisionGenotype> parents) {
        logger.info("BinaryBayesOperator.crossover: Entering ...");
        // TODO Auto-generated method stub
        final Adapter adapter = new Adapter();
        // Some trial method
        final DesignDecisionGenotype DDGTemplate = parents.get(0);
        final List<Choice> ChoiceTemplate = new ArrayList<Choice>();
        for(int j = 0 ; j < DDGTemplate.size() ; j++){
            ChoiceTemplate.add(DDGTemplate.get(j));
        }
        // Choice template done
        final List<FinalBinaryGenotype> FBGenotypeList = new ArrayList<FinalBinaryGenotype>();
        for(int i = 0 ; i < parents.size() ; i++){
            final List<BinaryGenotype> BGenotypeList = adapter.translateDesignDecisionGenotype(parents.get(i));
        ...
        final Integer[][] BinaryGenes = new Integer[FBGenotypeList.size()][FBGenotypeList.get(0).getBinaryGenotype().size()];
        ...
        final int[][] BinaryGenesint = new int[BinaryGenes.length][BinaryGenes[0].length];
        for(int i=0;i<BinaryGenes.length;i++){
            for(int j=0;j< BinaryGenes[0].length;j++){
                BinaryGenesint[i][j] = BinaryGenes[i][j];
            }
        }
        ...
        final List<FinalBinaryGenotype> FBGenotypeOffspringList = new ArrayList<FinalBinaryGenotype>();
        for(int i=0;i<Offspring.length;i++){
            final List<Integer> GeneratedOffspringList = new ArrayList<Integer>();
            for(int j=0;j<Offspring[0].length;j++){
                GeneratedOffspringList.add(Offspring[i][j]);
            }
            // Create an FBGenotype object
            final FinalBinaryGenotype FBOffspringObj = new FinalBinaryGenotype();
        ...
        final List<DesignDecisionGenotype> DDGenotypeOffspringList = new ArrayList<DesignDecisionGenotype>();
        for(int i = 0;i<FBGenotypeOffspringList.size();i++){
            final DesignDecisionGenotype ddg = adapter.translateFinalBinaryGenotype(FBGenotypeOffspringList.get(i));
            final DesignDecisionGenotype ddgpure = new DesignDecisionGenotype();
            for(int k = 0 ; k < ddg.size() ; k++){
                if(ChoiceTemplate.get(k).getDegreeOfFreedomInstance() instanceof ContinuousProcessingRateDegree){
                    final ContinousRangeChoice purechoice = designdecisionFactory.eINSTANCE.createContinousRangeChoice();
                    ...
                    final Choice purechoice = designdecisionFactory.eINSTANCE.createChoice();
                    purechoice.setDegreeOfFreedomInstance(ChoiceTemplate.get(k).getDegreeOfFreedomInstance());
                    purechoice.setIsActive(ChoiceTemplate.get(k).isActive());
                    purechoice.setValue(ddg.get(k).getValue());
                    ...
                    final ClassChoice purechoice = designdecisionFactory.eINSTANCE.createClassChoice();
                    purechoice.setDegreeOfFreedomInstance(ChoiceTemplate.get(k).getDegreeOfFreedomInstance());
                    purechoice.setIsActive(ChoiceTemplate.get(k).isActive());
                    purechoice.setValue(ddg.get(k).getValue());
                    ...
                    final DiscreteRangeChoice purechoice = designdecisionFactory.eINSTANCE.createDiscreteRangeChoice();
                    purechoice.setDegreeOfFreedomInstance(ChoiceTemplate.get(k).getDegreeOfFreedomInstance());
                    purechoice.setIsActive(ChoiceTemplate.get(k).isActive());
                    purechoice.setValue(ddg.get(k).getValue());
     ...
     */
    private int[][] getSampledGenomes(final int[][] currentGenomes){
        logger.info("BinaryBayesOperator.getSampledGenomes: Entering ...");
        // Part1: Learn Network Structure
        //<--------------------------------------------------------------------->
        final HillClimber hc = new HillClimber();
        final int[][] Graph = hc.search(currentGenomes, 5); // 2nd argument is the maximum-number-of-parents-any-node-can-have constraint.
        //<--------------------------------------------------------------------->
        ...
        //<--------------------------------------------------------------------->
        final int index = this.GraphNumber++;
        final String file_name = "C:/Users/Hp/runtime-EclipseApplication/Simple Tactics Example/PerOpteryx_results/Graph"+Integer.toString(index)+".txt";
        final WriteFile data = new WriteFile(file_name,true);
        ...
        //<--------------------------------------------------------------------->
        final BOAsampler boasampler = new BOAsampler(Graph, currentGenomes);
        final int[][] returnmatrix = boasampler.sample(100);
        logger.info("BinaryBayesOperator.crossover: Returning ...");
        return returnmatrix;
    }
