 ...
 * identifies the available design options to define the design space.
 *
 * @author Anne
 *
 */
public class DSEProblem {
    ...
    public DSEProblem(final DSEWorkflowConfiguration dseConfig, MDSDBlackboard blackboard) throws CoreException {
        ...
        if (newProblem) {
            ...
            // Alternative Component has to be called to create the mapping for all AssemblyContexts
            // and their roles,
        ...
        /*
         * Meta model design decisions?? Each design decision would know its bounds and the
         * annotated element or more specific the annotated property of this element that is exactly
         * the value range than the bound of the decision.
         *
         * But what about the genotype? With a ordered list of design decisions, I can map the
         * genotype entry to a design decisions. Better group in integer and double, then. ->
         * require CompositeGenotype
         *
    ...
    private DecisionSpace loadProblem(final URI filename) throws CoreException {
        ...
        if (!(eproblem instanceof DecisionSpace)) {
            throw new CoreException(new Status(Status.ERROR, "de.uka.ipd.sdq.dsexplore", 0,
        ...
        for (final DegreeOfFreedomInstance dd : problem.getDegreesOfFreedom()) {
            ...
            if (dd.getDof() != null) {
                ...
                if (value instanceof EObject) {
                    ...
                    throw new CoreException(new Status(Status.ERROR, "de.uka.ipd.sdq.dsexplore", 0,
                            "Cannot cast " + value
                                    + " to an EObject, Integer, or Double. Please extend DSEProblem.determineInitialGenotype to handle your type of choice.",
                            null));
                }
                ...
                if (degree instanceof CapacityDegree) {
                    ...
                    if (entity instanceof ResourceContainer) {
                        ...
                        if (StereotypeAPI.isStereotypeApplied(resourceContainer, "ReplicableResourceContainer")) {
                            ...
                            StereotypeAPI.setTaggedValue(resourceContainer, min, "ReplicableResourceContainer",
                                    "numberOfReplicas");
                            choice.setChosenValue(min);
                        }
                    } else if (entity instanceof AssemblyContext) {
                        //TODO: Add specific code to set numberOfReplicas of AssemblyContext
                    }
                } else {
                    throwUnknownDegreeException(dd);
                }
     ...
     *
     */
    // private void determineSOAPOrRMIDecisions(List<DesignDecision> dds, DesignDecisionGenotype
    // genotype) {
    //
    ...
    // TODO: change this to two visitors that either create the design decision and initial genotype
    // or just initial genotype.
    private void determineAllocationDecisions(final List<DegreeOfFreedomInstance> dds,
            final DesignDecisionGenotype genotype) {
        final List<AllocationContext> acs = this.initialInstance.getAllocation().getAllocationContexts_Allocation();
        final List<ResourceContainer> rcs = this.initialInstance.getResourceEnvironment()
    ...
    /**
     * Be sure to add one design decision and one gene in the initial genotype at once. The index is
     * important.
     * 
     * @param genotypeIndex
     */
    private void determineAssembledComponentsDecisions(final List<DegreeOfFreedomInstance> dds,
            final DesignDecisionGenotype genotype) {
    ...
    /**
     * Be sure to add one design decision and one gene in the initial genotype at once. The index is
     * important. Creates {@link ContinuousProcessingRateDegree} decisions for each found processing
     * resource.
     *
     ...
     */
    private void determineProcessingRateDecisions(final List<DegreeOfFreedomInstance> dds,
            final DesignDecisionGenotype genotype) {
        final List<ResourceContainer> resourceContainers = this.initialInstance.getResourceEnvironment()
                .getResourceContainer_ResourceEnvironment();
        for (final ResourceContainer resourceContainer : resourceContainers) {
            ...
            for (final ProcessingResourceSpecification resource : resources) {
                ...
                // XXX initial assumption: the highest possible processingRate is 10 times the
                // current one.
                ...
                // Create SchedulingPolicyDegree (excluded here, not in default problem, can be
                // modelled manually).
                /*
                 * SchedulingPolicyDegree schedulingDecision =
                 * this.designDecisionFactory.createSchedulingPolicyDegree(); SchedulingPolicy
                 * currentPolicy = resource.getSchedulingPolicy();
                 * 
                 * schedulingDecision.getDomainOfAllowedSchedulingPolicies().add(SchedulingPolicy.
                 * FCFS);
                 * schedulingDecision.getDomainOfAllowedSchedulingPolicies().add(SchedulingPolicy.
                 * PROCESSOR_SHARING); schedulingDecision.setPrimaryChanged(resourceContainer);
    ...
    /**
     * Returns the degree of freedom of the type (or subtype) that has the given entity as the
     * changeableEntity.
     * 
     * @param entity
     * @param degreeClass
     ...
     */
    public DegreeOfFreedomInstance getDegree(final Entity entity,
            final Class<? extends DegreeOfFreedomInstance> degreeClass) {
        ...
        for (final DegreeOfFreedomInstance DegreeOfFreedomInstance : degrees) {
            ...
            if (degreeClass.isInstance(DegreeOfFreedomInstance)
                    && DegreeOfFreedomInstance.getPrimaryChanged().equals(entity)) {
