 ...
 *
 */
public class DSEDecoder implements
		Decoder<DesignDecisionGenotype, PCMPhenotype> {
	 ...
	 * @param pcm
	 */
	private void applyChange(DegreeOfFreedomInstance designDecision,
			Choice choice, GenomeToCandidateModelTransformation trans,
			PCMInstance pcm) {
		...
		if (ProcessingRateDegree.class.isInstance(designDecision)) {
			this.applyChangeProcessingRateDecision(
					(ProcessingRateDegree) designDecision, choice);

			// Monitoring degree added
			// added by Suman Jojiju
		} else if (MonitoringDegree.class.isInstance(designDecision)) {
			this.applyChangeMonitoringDecision(
					(MonitoringDegree) designDecision, choice);

		} else if (AssembledComponentDegree.class.isInstance(designDecision)) {
			this.applyChangeAssembledComponentDecision(
					(AssembledComponentDegree) designDecision, choice);
		} else if (AllocationDegree.class.isInstance(designDecision)) {
			this.applyChangeAllocationDecision(
					(AllocationDegree) designDecision, choice);
		} else if (SchedulingPolicyDegree.class.isInstance(designDecision)) {
			this.applyChangeSchedulingDecision(
					(SchedulingPolicyDegree) designDecision, choice);
		} else if (CapacityDegree.class.isInstance(designDecision)) {
			this.applyChangeCapacityDecision((CapacityDegree) designDecision,
					choice);
		} else if (NumberOfCoresDegree.class.isInstance(designDecision)) {
			this.applyChangeNumberOfCoresDecision(
					(NumberOfCoresDegree) designDecision, choice);
		} else if (ResourceContainerReplicationDegree.class
				.isInstance(designDecision)) {
			this.applyChangeResourceContainerReplicationDegree(
					(ResourceContainerReplicationDegree) designDecision, choice);
		} else {
			try {
				trans.transformChoice(pcm, choice);
			} catch (Exception e) {
				logger.error("There was an unrecognised design decision "
						+ designDecision.getClass()
						+ " or a problem when handling a generic DoF");
				e.printStackTrace();
			}
		}
	}
	...
	private void applyChangeResourceContainerReplicationDegree(
		...
		if (!(changeableEntity instanceof ResourceContainer)) {
			throwInvalidEntityException(designDecision, changeableEntity,
		...
		List<ResourceContainer> allServers = pcm.getResourceEnvironment()
		...
		for (ResourceContainer resourceContainer : allServers) {
			if (resourceContainer.getId().contains(server.getId())
					&& resourceContainer.getEntityName().contains("Replica")) {
		...
		List<LinkingResource> linkingResources = pcm.getResourceEnvironment()
		...
		for (LinkingResource linkingResource : linkingResources) {
			linkingResource.getConnectedResourceContainers_LinkingResource()
					.removeAll(previousReplicasToRemove);
		}

		List<AllocationContext> allocationContextsToRemove = new ArrayList<AllocationContext>();
		for (AllocationContext allocationContext : pcm.getAllocation()
				.getAllocationContexts_Allocation()) {
			if (previousReplicasToRemove.contains(allocationContext
					.getResourceContainer_AllocationContext())) {
				allocationContextsToRemove.add(allocationContext);
			}
		}
		pcm.getAllocation().getAllocationContexts_Allocation()
				.removeAll(allocationContextsToRemove);
		...
		if (numberOfServers == 1) {
			...
			List<AllocationContext> allocationContexts = pcm.getAllocation()
					.getAllocationContexts_Allocation();
			List<AllocationContext> allocationContextsOnServer = getAllocationContextsOnServer(
					allocationContexts, server);

			// the initial server is already there, so add numberOfServers - 1
			// new servers
			for (int i = 1; i < numberOfServers; i++) {
				// add a server, replicate all components on it.

				ResourceContainer serverCopy = (ResourceContainer) EcoreUtil
						.copy(server);
				serverCopy.setId(serverCopy.getId() + i);
				serverCopy.setEntityName(serverCopy.getEntityName() + "Replica"
						+ i);
				pcm.getResourceEnvironment()
						.getResourceContainer_ResourceEnvironment()
						.add(serverCopy);

				// connect the new server copy to all linking resources that the
				// old one is connected to
				for (LinkingResource linkingResource : linkingResources) {
					if (linkingResource
				...
				for (AllocationContext allocationContext : allocationContextsOnServer) {
					AllocationContext allocationContextCopy = (AllocationContext) EcoreUtil
							.copy(allocationContext);
					allocationContextCopy.setId(allocationContextCopy.getId()
							+ i);
					allocationContextCopy.setEntityName(allocationContextCopy
							.getEntityName() + "Replica" + i);

					allocationContextCopy
							.setResourceContainer_AllocationContext(serverCopy);

					pcm.getAllocation().getAllocationContexts_Allocation()
							.add(allocationContextCopy);
				}
			}
		}
	}
	...
	private void applyChangeCapacityDecision(CapacityDegree designDecision,
		...
		PassiveResource passiveResource = (PassiveResource) designDecision
				.getPrimaryChanged();
		passiveResource.getCapacity_PassiveResource().setSpecification(
				String.valueOf(discreteChoice.getChosenValue()));

	}

	private void applyChangeAllocationDecision(AllocationDegree designDecision,
			Choice choice) {
		...
		if (!(entity instanceof ResourceContainer)) {
			throwInvalidEntityException(designDecision, entity,
					ResourceContainer.class);
		}

		ResourceContainer rc = (ResourceContainer) entity;

		((AllocationContext) designDecision.getPrimaryChanged())
				.setResourceContainer_AllocationContext(rc);

		logger.debug("Handling a " + designDecision.getClass()
				+ ", setting resource container to " + rc.getEntityName()
	...
	private void applyChangeSchedulingDecision(
		...
		SchedulingPolicy chosenPolicy = (SchedulingPolicy) schedChoice
				.getChosenValue();
	 ...
	 * @param doubleGene
	 */
	private void applyChangeProcessingRateDecision(
			ProcessingRateDegree designDecision, Choice choice) {
		...
		if (choice instanceof ContinousRangeChoice) {
			...
			newRate = degree.getFrom()
					+ ((degree.getTo() - degree.getFrom()) / degree
							.getNumberOfSteps()) * doubleGene.getChosenValue();
		...
		if (Double.isNaN(this.initialRate)) {
			...
			String oldRateString = rightPrs
					.getProcessingRate_ProcessingResourceSpecification()
					.getSpecification();
			this.initialRate = CostUtil.getInstance()
					.getDoubleFromSpecification(oldRateString);
		}
		...
		rightPrs.getProcessingRate_ProcessingResourceSpecification()
				.setSpecification(String.valueOf(newRate));
		rightPrs.setMTTF(mttf);

		logger.debug("Handling a " + designDecision.getClass()
				+ ", setting rate to " + newRate + " and MTTF to " + mttf
				+ " (inital MTTF: " + this.initialMTTF + ")");
	}

	// added by Suman Jojiju

	private void applyChangeMonitoringDecision(MonitoringDegree designDecision,
			Choice choice) {
		if (choice instanceof ContinousRangeChoice) {
			ContinousRangeChoice doubleGene = (ContinousRangeChoice) choice;
			interval = (Intervall) designDecision.getPrimaryChanged();
			interval.setIntervall(doubleGene.getChosenValue());
	...
	private ProcessingResourceSpecification getProcessingRateSpecification(
		...
		ResourceContainer rc = ((ResourceContainer) designDecision
				.getPrimaryChanged());
		ProcessingResourceType prt = designDecision.getProcessingresourcetype();

		ProcessingResourceSpecification rightPrs = null;
		for (ProcessingResourceSpecification prs : rc
				.getActiveResourceSpecifications_ResourceContainer()) {
			if (EMFHelper.checkIdentity(
					prs.getActiveResourceType_ActiveResourceSpecification(),
					prt)) {
				rightPrs = prs;
				break;
			}
		...
		if (rightPrs == null) {
			throw new RuntimeException(
					"Invalid degree of freedom "
							+ designDecision.toString()
							+ ". The references ProcessingResourceType is not available in the given ResourceContainer.");
		}
		return rightPrs;
	}
	...
	private void applyChangeAssembledComponentDecision(
			AssembledComponentDegree designDecision, Choice choice) {
		...
		AssemblyContext changedAssemblyContext = (AssemblyContext) designDecision
				.getPrimaryChanged();
		RepositoryComponent currentComponent = changedAssemblyContext
		...
		if (!EMFHelper.checkIdentity(currentComponent, componentToBeAssembled)) {
			AlternativeComponent.getInstance().applyChange(
					changedAssemblyContext, componentToBeAssembled);
		}

		logger.debug("Handling a " + designDecision.getClass()
				+ ", using component " + componentToBeAssembled.getEntityName());
	}

	private void throwInvalidEntityException(
			DegreeOfFreedomInstance designDecision, EObject changeableEntity,
			Class<?> expectedClass) {
		throw new RuntimeException("Entity " + changeableEntity + " of type "
				+ changeableEntity.getClass().getName()
				+ " is not an applicable value for degree of freedom "
				+ designDecision.getClass().getName() + ", required type is "
	 ...
	 */
	public static Choice getChoiceForIndex(int index, String decisionString)
			throws CoreException {

		DegreeOfFreedomInstance designDecision = Opt4JStarter.getProblem()
				.getDesignDecision(index);
	...
	public static Choice getChoiceFor(String decisionString,
		...
		if (designDecision instanceof ContinuousRangeDegree) {
			...
			if (!validRange(d, contDegree, contDegree.getFrom(),
					contDegree.getTo())) {
				throw ExceptionHelper.createNewCoreException("Error: Value "
						+ d
						+ " is not a valid value for degree "
						+ designDecision
						+ " "
						+ DegreeOfFreedomHelper
								.getDegreeDescription(designDecision));
			}
			ContinousRangeChoice contChoice = factory
			...
			if (!validRange(i, discDegree, discDegree.getFrom(),
					discDegree.getTo())) {
				throw ExceptionHelper.createNewCoreException("Error: Value "
						+ i
						+ "\" is not a valid value for degree "
			...
			DiscreteRangeChoice discChoice = factory
			...
			Entity entity = getEntityFor((ClassDegree) designDecision,
					decisionString);
			if (entity == null) {
				throw ExceptionHelper
						.createNewCoreException("Error: Decision string \""
								+ decisionString
								+ "\" is not a valid value for degree "
								+ designDecision
								+ " "
			...
			List<EObject> options = ((SchedulingPolicyDegree) designDecision)
			...
			logger.warn("There was an unrecognised design decision "
					+ designDecision.getClass());
			return null;
		}
	...
	private static boolean validRange(double value, RangeDegree designDecision,
	 ...
	 * @param choice
	 */
	public static void applyChangeToGenotype(DesignDecisionGenotype genotype,
			DegreeOfFreedomInstance degree, Choice choice) {
		int index = Opt4JStarter.getProblem().getDesignDecisions()
				.indexOf(degree);
