...
public class EMFHelper {
	...
	private static void saveToXMIFile(final EObject modelToSave, final URI myURI, boolean mayRetry) {
		...
		try {
			...
			if (mayRetry && (myURI.toFileString().length() > 250)) {
				// try again with a shorter filename, but just one more try
				// (mayRetry = false).
				...
				myShorterURI = myShorterURI
						.appendSegment(lastSegmentShortened + "-shortened-" + myURI.toString().hashCode());
	...
	public static List<PassiveResource> getPassiveResources(final List<Repository> repositoryList) {
		...
		for (final Repository repository : repositoryList) {
			final List<RepositoryComponent> repoComponents = repository.getComponents__Repository();
	...
	public static List<AllocationContext> getAllUsedAllocationContexts(final Allocation allocation) {
