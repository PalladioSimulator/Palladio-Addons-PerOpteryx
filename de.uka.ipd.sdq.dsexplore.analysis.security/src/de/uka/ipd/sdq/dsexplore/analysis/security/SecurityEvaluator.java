...
public class SecurityEvaluator extends AbstractAnalysis implements IAnalysis {
	...
	public void analyse(final PCMPhenotype pheno, final IProgressMonitor monitor)
		...
		final ResourceContainer dmzServerCandidate = allocationContextOfWebserver.getResourceContainer_AllocationContext();

		for (final AllocationContext allocationContext : allocation.getAllocationContexts_Allocation()) {
			if (allocationContext != allocationContextOfWebserver && allocationContext.getResourceContainer_AllocationContext() == dmzServerCandidate
		...
		this.previousSecurityResults.put(pheno.getNumericID(), new SecurityAnalysisResult(securityValue, pcm, this.criterionToAspect,
				(SecuritySolverQualityAttributeDeclaration) this.qualityAttribute));
	}
	...
	@Override
	public IAnalysisResult retrieveResultsFor(final PCMPhenotype pheno, final Criterion criterion) throws CoreException, AnalysisFailedException {
