...
public class PcmSystemManager {
	...
	public static PcmSystemManager getInstanceBy(System system) {
		if (PcmSystemManager.eInstance == null) {
			PcmSystemManager.eInstance = new PcmSystemManager();
		}
		PcmSystemManager.eInstance.system = system;
		return PcmSystemManager.eInstance;
	}
	...
	public List<Connector> getConnectorsBy(Predicate<Connector> searchCriteria) {
		...
		return result;
		// return
		// this.getAllConnectors().filter(searchCriteria).collect(Collectors.toList());
	}
