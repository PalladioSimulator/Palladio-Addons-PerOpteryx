...
public class ErrorMessage {
	...
	public static String missingExternalCall(RepositoryComponent component, Signature calledService) {
		return String.format("Component %1s contains no external call to service %2s.", component.getEntityName(), calledService.getEntityName());
	}
	...
	public static String missingECC(ElementaryConcernComponent ecc, List<ProvidedRole> providedFeatures) {
		return String.format("For ecc %1s does not contain the provided features:%2s.", ecc.getName(), ErrorMessage.toString(providedFeatures));
	}
	...
	private static <T extends NamedElement> String toString(List<T> namedElements) {
		return namedElements.stream().map(eachElement -> eachElement.getEntityName()).reduce("", (s1, s2) -> String.format("%1s, %2s", s1, s2));
	}
