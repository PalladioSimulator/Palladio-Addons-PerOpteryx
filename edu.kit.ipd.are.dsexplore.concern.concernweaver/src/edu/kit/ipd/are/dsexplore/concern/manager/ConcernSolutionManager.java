...
public class ConcernSolutionManager {
	 ...
	 * @param concernSolution - A given concern solution.
	...
	public static ConcernSolutionManager getInstanceBy(Repository concernSolution) {
		...
		if (eInstance == null) {
			...
			eInstance = new ConcernSolutionManager();
	 ...
	 * @param searchCriteria - The search criteria for filtering the components.
	...
	public List<RepositoryComponent> getComponentsAnnotatedWith(List<Annotation> annotations) {
		...
		return new AnnotationFilter(Arrays.asList(concernSolution)).getComponentsAnnotatedWith(annotations);
		
	}
	
	private Stream<RepositoryComponent> getAllComponents() {
		
		return this.concernSolution.getComponents__Repository().stream();
		
	}
	
	/**
	 * Creates and add a adapter component to the concern solution repository.
	 * @param name - The name of the adapter component.
	...
	/**
	 * Creates a required role based on the complimentary operation provided role.
	 * @param providedRole - The complimentary operation provided role.
	 ...
	 * @param sinkRole - The complimentary provided sink role.
	 * @return the created required role.
	 */
	...
	/**
	 * Creates a provided role based on the complimentary operation required role.
	 * @param requiredRole - The complimentary operation required role.
	 ...
	 * @param sourceRole - The complimentary required source role.
	 * @return the created provided role.
	 */
	...
	/**
	 * Creates a provided role based on a provided interface of another operation provided role.
	 * @param sourceRole - The operation provided role referencing the provided interface.
	 * @return the created provided role.
	 */
	...
	/**
	 * Creates the provided role space which contains all provided roles that occur in the concern solution repository.
	 * @return the provided role space.
	 */
	public List<ProvidedRole> getAllProvidedRoles() {
		
		return this.concernSolution.getComponents__Repository().stream().flatMap(eachComponent -> getAllProvidedRolesOf(eachComponent))
