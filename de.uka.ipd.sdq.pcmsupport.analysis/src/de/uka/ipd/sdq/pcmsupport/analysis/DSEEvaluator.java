package de.uka.ipd.sdq.pcmsupport.analysis;
 ...
 * 
 */
public class DSEEvaluator implements de.uka.ipd.sdq.dsexplore.opt4j.representation.DSEEvaluator<PCMPhenotype> {
	...
	private static void copyPCMPartitionToAnalysisSlot(MDSDBlackboard blackboard) {
		...
		PCMResourceSetPartition analysisPartition = (PCMResourceSetPartition)blackboard.getPartition(LoadModelIntoBlackboardJob.MODEL_PARTITION_ID);
		ResourceSet analysisResourceSet = analysisPartition.getResourceSet();
		
		// clear any previous content.
		analysisResourceSet.getResources().clear();
		
		PCMResourceSetPartition originalModelPartition = (PCMResourceSetPartition)blackboard.getPartition(MoveInitialModelPartitionJob.INITIAL_MODEL_PARTITION_ID);
