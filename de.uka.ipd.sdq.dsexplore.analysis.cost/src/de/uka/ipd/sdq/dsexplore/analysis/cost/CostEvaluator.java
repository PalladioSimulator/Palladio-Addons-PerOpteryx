...
public class CostEvaluator extends AbstractAnalysis implements IAnalysis {
	...
	public void analyse(PCMPhenotype pheno, IProgressMonitor monitor)
		...
		this.previousCostResults.put(pheno.getNumericID(),
				new CostAnalysisResult(CostUtil.getTotalCost(initialCost, operatingCost, costModel.getInterest(), costModel.getTimePeriodYears()),
				...
				if (resourceContainer.getEntityName().contains("Replica") && resourceContainer.getId().contains(originalContainer.getId())) {
					...
					for (Cost existingCost : allCosts) {
						if (existingCost instanceof ProcessingResourceCost) {
							...
							if (existingProcRateCost.getProcessingresourcespecification().getResourceContainer_ProcessingResourceSpecification()
									.getId().equals(resourceContainer.getId())) {
					...
					if (replicaProcSpec == null) {
						logger.warn("Could not find processing resource type " + procResourceType.getEntityName() + " in container "
								+ resourceContainer.getEntityName() + ", assuming that there are no costs for it in this replica");
						return;
					}
	...
	private CostRepository getCostModel(DSEWorkflowConfiguration configuration) throws CoreException {
		...
		if (cr == null) {
			throw new CoreException(
					new Status(Status.ERROR, "de.uka.ipd.sdq.dsexplore", 0, "Cost model " + costModelFileName + " could not be loaded.", null));
		}
		return cr;
	}
