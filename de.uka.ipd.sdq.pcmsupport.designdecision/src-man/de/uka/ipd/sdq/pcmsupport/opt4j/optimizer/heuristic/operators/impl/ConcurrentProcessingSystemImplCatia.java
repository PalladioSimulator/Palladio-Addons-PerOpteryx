...
public class ConcurrentProcessingSystemImplCatia extends APCMTactic {
	...
	public List<TacticsResultCandidate> getSolution(DSEIndividual i) {
		...
		if (performance != null
			...
			PCMInstance pcm = ((DSEProblem) UniversalDoF.eINSTANCE.getTarget()).getPcmInstance();
			...
			if (choice instanceof ClassChoice) {
				...
				if (classChoice.getDofInstance() instanceof AllocationDegree) {
					...
					if (EMFHelper.checkIdentity(
							classChoice.getDofInstance().getPrimaryChanged(), 
							result.getFirst().ac)) {

						// if (ClassChoice.getDegreeOfFreedomInstance().g)

						classChoice.setChosenValue(EMFHelper.retrieveEntityByID(
								((AClassDegree)classChoice.getDofInstance()).getClassDesignOptions(), 
								result.getSecond()));
	...
	private TacticsResultCandidate createIncreasedCapacityCandidate(
		...
		for (Choice choice : candidate.getGenotype()) {
			if (choice instanceof DiscreteRangeChoice) {
				...
				if (schedChoice.getDofInstance() instanceof CapacityDegree) {

					if (EMFHelper.checkIdentity(schedChoice.getDofInstance().getPrimaryChanged(), pr)) {
	...
	private TacticsResultCandidate createUpdatedSchedulingCandidate(
		...
		for (Choice choice : candidate.getGenotype()) {
			if (choice instanceof ClassChoice) {
				ClassChoice classChoice = (ClassChoice) choice;
				if (classChoice.getDofInstance() instanceof SchedulingPolicyDegree) {

					SchedulingPolicyDegree schedDegree = (SchedulingPolicyDegree)classChoice.getDofInstance();
