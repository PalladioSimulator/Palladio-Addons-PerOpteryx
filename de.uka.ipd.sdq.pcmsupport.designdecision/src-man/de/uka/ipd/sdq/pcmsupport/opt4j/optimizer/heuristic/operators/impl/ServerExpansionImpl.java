 ...
 */
public class ServerExpansionImpl extends APCMTactic {
	/**
	...
	private void createServerMultiplicityExpansionCandidates(
		...
		for (Choice choice : candidate.getGenotype()){
			if (choice instanceof DiscreteRangeChoice && choice.getDofInstance() instanceof ResourceContainerReplicationDegree){
				ResourceContainerReplicationDegree replDegree = (ResourceContainerReplicationDegree)choice.getDofInstance();
	...
	private void createClassicServerExpansionCandidates(
		...
		for (int i = 0; i < reallocateAtMost && componentsAllocatedHere.size() > 0; i++){ 
			...
			EObject targetResourceContainerInMemory = EMFHelper.retrieveEntityByID(((AClassDegree)classChoice.getDofInstance()).getClassDesignOptions(),	targetResourceContainer);
