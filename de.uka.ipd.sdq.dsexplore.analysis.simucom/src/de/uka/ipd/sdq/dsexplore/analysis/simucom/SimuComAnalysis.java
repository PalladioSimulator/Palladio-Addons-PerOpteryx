...
public class SimuComAnalysis extends AbstractAnalysis implements IAnalysis {
    ...
    /**
     * Calls SimuCom. Before doing so, it calls the {@link ConfigurationHelper} to update the
     * {@link ILaunchConfiguration} and stores the {@link PCMInstance} to files, so that SimuCom can
     * read it as usual. After the SimuCom run, the analysis results are extracted from the
     * sensorFramework data sources and returned. The returned {@link IAnalysisResult} is a
     * {@link SimuComAnalysisResult} which does not store the results directly, but provides access
     * to the underlying sensorFramework data sources.
     *
     * {@inheritDoc}
     * 
    ...
    /**
     * Search in all open data sources whether there is already an experiment run with this name and
     * check that it contains some results.
     * 
     * @param experimentSettingName
     * @return
     */
    private boolean isExperimentRunDoesNotExist(final String experimentName, final String experimentSettingName)
            throws CoreException {

        if (!config.getAttribute("persistenceFramework", "")
                .equals(org.palladiosimulator.recorderframework.edp2.Activator.EDP2_ID)) {
            throw new RuntimeException("Persistence Framework has to be EDP2");
        }
    ...
    /**
     * FIXME: This method should not depend on the state of the blackboard anymore... but it does at
     * this time.
     *
     * @param pheno
     * @param entity
    ...
    private IStatisticAnalysisResult retrieveSimuComResults(final PCMPhenotype pheno, final Entity entity)
        ...
        result = SimuComAnalysisEDP2Result.findExperimentRunAndCreateResult(entity, experimentName,
                experimentSettingName, pcmInstance, selectedRepo, this.criterionToAspect,
                (SimuComQualityAttributeDeclaration) this.qualityAttribute);

        if (result == null) {
            final String errormessage = "There was no experiment named \"" + experimentName
                    + "\" with an experiment setting \"" + experimentSettingName + "\""
                    + " in the selected data source after analysing the PCM instance \"" + experimentName
    ...
    private void launchSimuCom(final IProgressMonitor monitor)
        ...
        // retry simulation if the cause was that an extension could not be loaded, because that
        // seems to be a transient problem (it only happens sometimes)
        ...
        for (int i = 0; i < numberOfTries; i++) {
            try {
                ...
                if (e.getCause() != null) {
                    ...
                    if (numberOfTries > 0 && causingErrorMessage != null
                            && causingErrorMessage.contains("Couldn't find extension")) {
                        logger.warn("Trying to start SimuCom again.");
                        continue;
                    }
                }
                // try to roll back and clean up (e.g. delete temporary folder). This is not tested
                // yet and may cause problems.
                try {
                    job.cleanup(monitor);
                } catch (CleanupFailedException e1) {
                    logger.error(
                            "Cleanup of failed simucoim run failed, probably you need to clean up manually (e.g. delete temorary plugin);");
                    e1.printStackTrace();
                }
    ...
    private Entity getPCMEntityForCriterion(final Criterion criterion) throws CoreException {
        ...
        throw new CoreException(new Status(Status.ERROR, "de.uka.ipd.sdq.dsexplore.analysis.simucom",
                "Cannot handle Criterion of type " + criterion.getClass()
                        + ". Required is UsageScenarioBasedCriterion or EntryLevelSystemCallCriterion."));
    }
    ...
    public boolean hasStatisticResultsFor() throws CoreException {
        // FIXME: need to properly map between criterion and the quality attribute dimension. Cannot
        // fix this before PALLADIO-384 is fixed.
        return true;
    }
