...
public class SimuComAnalysis extends AbstractAnalysis implements IAnalysis {
	...
	private boolean isExperimentRunDoesNotExist(final String experimentName, final String experimentSettingName) throws CoreException {
		...
		if (config.getAttribute("persistenceFramework", "").equals("SensorFramework")) {
			return !SimuComAnalysisSensorFrameworkResult.isExperimentRunExisting(experimentName, this.simuComWorkflowConfiguration,
					this.datasourceReloadCount, config);
		}
		// In case the selected persistence framework is EDP2
		else {
			return !SimuComAnalysisEDP2Result.isExperimentRunExisting(experimentName, experimentSettingName,
					SimuComAnalysisEDP2Result.findSelectedEDP2Repository(config));
		}
	...
	/**
	 * FIXME: This method should not depend on the state of the blackboard
	 * anymore... but it does at this time.
	 *
	 * @param pheno
	 * @param entity
	 * @return
	...
	private IStatisticAnalysisResult retrieveSimuComResults(final PCMPhenotype pheno, final Entity entity)
		...
		if ("SensorFramework".equals(config.getAttribute("persistenceFramework", ""))) {
			result = new SimuComAnalysisSensorFrameworkResult(entity, experimentName, pcmInstance, this.criterionToAspect,
					(SimuComQualityAttributeDeclaration) this.qualityAttribute, this.config);

			if (result == null) {
				final String errormessage = "There was no experiment run or no experiment for experiment named \"" + experimentName
						+ "\" in the selected data source after analysing the PCM instance \"" + experimentName + "\" of candidate "
						+ pheno.getNumericID() + " " + pheno.getGenotypeID();
		...
		// In case the selected persistence framework is EDP2
			...
			result = SimuComAnalysisEDP2Result.findExperimentRunAndCreateResult(entity, experimentName, experimentSettingName, pcmInstance,
					selectedRepo, this.criterionToAspect, (SimuComQualityAttributeDeclaration) this.qualityAttribute);

			if (result == null) {
				final String errormessage = "There was no experiment named \"" + experimentName + "\" with an experiment setting \""
						+ experimentSettingName + "\"" + " in the selected data source after analysing the PCM instance \"" + experimentName
						+ "\" of candidate " + pheno.getNumericID() + " " + pheno.getGenotypeID();
				logger.error(errormessage);
				throw new AnalysisFailedException(errormessage);
			}
	...
	private void launchSimuCom(final IProgressMonitor monitor) throws CoreException, AnalysisFailedException, UserCanceledException {

		/*
		 * LoadPCMModelsIntoBlackboardJob loadJob = new
		 * LoadPCMModelsIntoBlackboardJob(config);
		 * 
		 * PCMInstance pcm = new
		 * PCMInstance((PCMResourceSetPartition)this.blackboard.getPartition(
		 * LoadPCMModelsIntoBlackboardJob.PCM_MODELS_PARTITION_ID));
		 * pcm.saveToXMIFile(pcm.getSystem(),
		 * this.config.getAttribute(ConstantsContainer.SYSTEM_FILE, ""));
		 * pcm.saveToXMIFile(pcm.getAllocation(),
		 * this.config.getAttribute(ConstantsContainer.ALLOCATION_FILE, ""));
		 * 
		...
		// loaded, because that seems to be a transient problem (it only happens
		// sometimes)
		...
		for (int i = 0; i < numberOfTries; i++) {
			try {
				...
				// try to roll back and clean up (e.g. delete temporary folder).
				// This is not tested yet and may cause problems.
				try {
					job.cleanup(monitor);
	...
	// /** Put all the old appends back in the logger. FIXME: does not work as
	// expected :( but whatever.
	// * @throws CoreException */
	// private void restoreLogger(ILaunchConfiguration config) throws
	// CoreException {
	...
	public void initialise(final DSEWorkflowConfiguration configuration) throws CoreException {
		...
		if (blackboard == null) {
			throw ExceptionHelper.createNewCoreException(
					"Error in initialisation: No Blackboard was set when initialising the SimuCom Analysis. Contact the developers.");
		}
	...
	@Override
	public IStatisticAnalysisResult retrieveResultsFor(final PCMPhenotype pheno, final Criterion criterion)
			throws CoreException, AnalysisFailedException {
	...
	private Entity getPCMEntityForCriterion(final Criterion criterion) throws CoreException {
		...
		throw new CoreException(new Status(Status.ERROR, "de.uka.ipd.sdq.dsexplore.analysis.simucom", "Cannot handle Criterion of type "
				+ criterion.getClass() + ". Required is UsageScenarioBasedCriterion or EntryLevelSystemCallCriterion."));
	}
	...
	public boolean hasStatisticResultsFor() throws CoreException {
		// FIXME: need to properly map between criterion and the quality
		// attribute dimension. Cannot fix this before PALLADIO-384 is fixed.
		return true;
	}
